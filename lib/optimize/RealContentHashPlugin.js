			...
			compilation.hooks.processAssets.tapPromise(
				...
				async () => {
					...
					const walkHashes = (
						...
						try {
							const handler = (hashes, cb) => hashes && hashes.forEach(cb);
							const comments = [];
							...
							walk.simple(ast, {
								Literal(/** @type {import('estree').Node | any} */ node) {
									...
									handler(`${value}`.match(hashRegExp), hash =>
										inContent(hash, raw, typeof value === "string", start, end)
									);
								},
							...
							comments.forEach(comment => {
								const { value, type, start, end } = comment;
								handler(value.match(hashRegExp), hash => {
									inContent(
										hash,
