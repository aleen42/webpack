			...
			compilation.hooks.processAssets.tapPromise(
				...
				async () => {
					...
					const hashRegExp = new RegExp(
						Array.from(hashToAssets.keys(), str => {
							str = quoteMeta(str);
							return /^[a-z0-9]+$/i.test(str) ? `\\b${str}\\b` : str;
						}).join("|"),
						"g"
					);
					const walkHashes = (
						content,
						/** @type {function(string, boolean, number, number)} */ inContent
					) => {
						try {
							const handler = (str, cb) => {
								let result;
								while ((result = hashRegExp.exec(str))) {
									cb(result[0], result.index);
								}
							};
							const comments = [];
							...
							walk.simple(ast, {
								Literal(/** @type {import('estree').Node | any} */ node) {
									const { raw, value, start } = node;
									handler(raw, (hash, index) =>
										inContent(hash, typeof value === "string", start, index)
									);
								},
								Property(/** @type {import('estree').Node | any} */ node) {
									const { name, raw, value, type, start } = node;
									handler(raw || name, (hash, index) => {
										inContent(
											hash,
							...
							comments.forEach(comment => {
								const { value, type, start } = comment;
								handler(
									type === "Line" ? `//${value}` : `/*${value}*/`,
									(hash, index) => {
										inContent(hash, true, start, index);
									}
								);
							});
						} catch (ignore) {
							// cannot parse asset into JavaScript AST
							let result;
							while ((result = hashRegExp.exec(content))) {
								inContent(result[0], true, 0, result.index);
							}
						}
					};

					const replaceHashes = (content, ownHashes) => {
						const positions = /** @type [{start: any, end: any}] */ [];
						walkHashes(content, (hash, quoted, start, index) => {
							const wrap = str => (quoted ? str : `'${str}'`);
							positions.push({
								start: start + index,
								end: start + index + hash.length,
								hash: wrap(
									ownHashes && ownHashes.has(hash)
										? ""
						...
						return (
							positions
								...
								.reduce(
									(str, { start, end, hash }, i, arr) =>
										str +
										content.slice((arr[i - 1] || { end: 0 }).end, start) +
										hash,
									""
								) + content.slice(positions[positions.length - 1].end)
						);
					};
					await Promise.all(
						...
						return (asset.contentComputePromise = (async () => {
							if (
								...
								asset.newSource = await cacheGenerate.providePromise(
									identifier,
									etag,
									() => new RawSource(replaceHashes(asset.content))
								);
							}
						})());
					};
						...
						return (asset.contentComputeWithoutOwnPromise = (async () => {
							if (
								...
								asset.newSourceWithoutOwn = await cacheGenerate.providePromise(
									...
									etag,
									() =>
										new RawSource(replaceHashes(asset.content, asset.ownHashes))
								);
							}
						})());
					};
