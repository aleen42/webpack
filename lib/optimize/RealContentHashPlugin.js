		...
		compiler.hooks.compilation.tap("RealContentHashPlugin", compilation => {
			...
			compilation.hooks.processAssets.tapPromise(
				...
				async () => {
					...
					const hashRegExp = RealContentHashPlugin.hashRegExp(
						hashToAssets.keys()
					);
					const hashReplacer = RealContentHashPlugin.hashReplacer(
						hashToAssets.keys()
					);
					await Promise.all(
...
${referencingAssets
							...
							if (
								...
								asset.newSourceWithoutOwn = await cacheGenerate.providePromise(
									...
									() => {
										const newContent = asset.content.replace(
											hashReplacer,
											hashReplacement(asset.ownHashes)
										);
										return new RawSource(newContent);
									}
								);
	...
	static hashRegExp(keys) {
		return new RegExp(Array.from(keys, quoteMeta).join("|"), "g");
	}

	static hashReplacer(keys) {
		return new RegExp(
			`(\\(?)(${Array.from(keys, quoteMeta).join("|")})\\)?`,
			"g"
		);
	}

	static hashReplacement(hashToNewHash) {
		return ownHashes => (ignore, caller, hash) => {
			if (ownHashes && ownHashes.has(hash)) {
				return "";
			}
			const newHash = hashToNewHash.get(hash);
			return /[^\d]/.test(newHash) && caller ? `('${newHash}')` : newHash;
		};
	}
}
