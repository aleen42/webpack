		...
		compiler.hooks.compilation.tap("RealContentHashPlugin", compilation => {
			...
			compilation.hooks.processAssets.tapPromise(
				...
				async () => {
					...
					const hashKeys = Array.from(hashToAssets.keys(), quoteMeta);
					const hashRegExp = new RegExp(`\\b${hashKeys.join("|")}\\b`, "g");
					const walkHashes = (
						content,
						/** @type {function(string, string, boolean, number, number)} */ inContent
					) => {
						try {
							const ast = acorn.parse(content, { ecmaVersion: 11 });
							walk.simple(ast, {
								Literal(/** @type {import('estree').Literal | any} */ node) {
									const { raw, value, start, end } = node;
									const hash = `${value}`.match(hashRegExp);
									if (hash) {
										inContent(
											hash[0],
											raw,
								...
								Property(/** @type {import('estree').Node | any} */ node) {
									const { name, value, type, start, end } = node;
									const hash = (
										{
											// {123: 'xxx'} or {'123': 'xxx'}
											Literal: `${value}`,
					...
					const replaceHashes = (content, ownHashes) => {
						...
						walkHashes(content, (hash, raw, quoted, start, end) => {
	...
	.join("\n")}`);
					...
					for (const oldHash of hashesInOrder) {
						...
						if (!newHash) {
							...
							// confirm it is not an only-numeric hash
							if (
								!isNaN(+newHash - parseFloat(newHash)) ||
								/^\d+n?$/.test(newHash)
							) {
								newHash = `a${newHash.slice(1)}`;
							}
						}
